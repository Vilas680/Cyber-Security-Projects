...
def analyze_text(text):
    """Checks input text against security keywords."""
    text = text.lower()
    alerts = []
...
def chatbot_answer(text):
    """Checks input text for common cyber security questions."""
    text = text.lower()
...
def simulate_vulnerability_scan(target):
    print(f"\n[SCAN] Scan for target: **{target}**")
    print("[SCAN] Checking for common security misconfigurations...")

    vulnerabilities = {
        "SQL Injection Potential": random.choice([True, False]),
        "Insecure HTTP Headers": random.choice([True, False]),
        "Open SSH Port (22)": random.choice([True, False]),
        "Default Credentials Found": random.choice([True, False, False]),
        "Cross-Site Scripting (XSS) Potential": random.choice([True, False]),
        "Outdated Server Software (Apache/Nginx)": random.choice([True, False, False, False]),
        "Insecure API Endpoint Exposed": random.choice([True, False]),
        "S3 Bucket Misconfiguration (Public Access)": random.choice([True, False, False, False, False]),
        "Sensitive Data in Configuration Files": random.choice([True, False]),
        "CORS Misconfiguration (Wildcard Origin)": random.choice([True, False, False]),
        "Exposed Git/SVN Repository": random.choice([True, False, False, False]),
    }

    found_vulnerabilities = [k for k, v in vulnerabilities.items() if v]

    if found_vulnerabilities:
        print("\n**🚨 Vulnerabilities Detected (Simulated): 🚨**")
        for vuln in found_vulnerabilities:
            print(f"  - **{vuln}**: Needs immediate attention!")
            
print ("\nChatbot Activated! Scanning for cybercrime indicators and accepting scan commands...\n")
print("Type 'exit' to stop.\n")

while True:
    user_input = input("Enter text to scan or a scan command: ")

    if user_input.lower() == "exit":
        print("Chatbot stopped. Stay safe online!")
        break

    if "scan-target" in user_input.lower():
        try:
            target = user_input.split("scan-target")[-1].strip().split()[0]
            if not target:
                print("Please provide a target fo0r the scan (e.g., scan-target example.com).")
                continue
            ...
            print("Invalid scan command format. Use: scan-target <target_address>")
            continue
