const keywords = {
    ...
    "Vulnerability Scan Command": ["scan-target", "check-security", "run-vulnerability-scan"]
};
...
function chatbotAnswer(text) {
    const lowerText = text.toLowerCase();
    for (const [question, answer] of Object.entries(cyberResponses)) { 
...
function simulateVulnerabilityScan(target) {
    const vulnerabilities = {
        "SQL Injection Potential": Math.random() < 0.5,
        "Insecure HTTP Headers": Math.random() < 0.5,
        "Open SSH Port (22)": Math.random() < 0.5,
        "Default Credentials Found": Math.random() < 0.33, 
        "Cross-Site Scripting (XSS) Potential": Math.random() < 0.5,
        "Outdated Server Software (Apache/Nginx)": Math.random() < 0.25,
        "Insecure API Endpoint Exposed": Math.random() < 0.5,
        "S3 Bucket Misconfiguration (Public Access)": Math.random() < 0.2, 
        "Sensitive Data in Configuration Files": Math.random() < 0.5,
        "CORS Misconfiguration (Wildcard Origin)": Math.random() < 0.35,
        "Exposed Git/SVN Repository": Math.random() < 0.25,
    };


    const foundVulnerabilities = Object.entries(vulnerabilities)
        .filter(([key, value]) => value)
        .map(([key]) => key);

    let output = `<div class="scan-box">
                    <strong>[SCAN] scan for target: ${target}</strong>
                    <p>[SCAN] Checking for common security misconfigurations...</p>`;

    if (foundVulnerabilities.length > 0) {
        output += `<div class="scan-alert">
                    <strong>🚨 Vulnerabilities Detected (Simulated): 🚨</strong>
                    <ul>`;
        foundVulnerabilities.forEach(vuln => {
            output += `<li>- <strong>${vuln}</strong>: Needs immediate attention!</li>`;
        });

    } else {
        output += `<p class="scan-success">✅ Simulated Scan Complete: No major vulnerabilities detected.</p>`;
    }
    output += `</div>`;
    return output;
}
...
function addAlertBox(alerts) {
    const chatBox = document.getElementById('chatBox');
    const messageDiv = document.createElement('div');
    messageDiv.className = 'message';

    const score = alerts.length * 15;
    ...
    alertBox.innerHTML = '<strong>⚠️ Suspicious Content Found:</strong>';

    alerts.forEach(alert => {
        const alertItem = document.createElement('div');
        alertItem.className = 'alert-item';
        alertItem.textContent = `• ${alert.word} (${alert.category})`;
        alertBox.appendChild(alertItem);
    });

    const severityDiv = document.createElement('div');
    severityDiv.className = `severity ${severityClass}`;
    severityDiv.textContent = `Severity Level → ${severity}`;
    alertBox.appendChild(severityDiv);
...
function sendMessage() {
    ...
    setTimeout(() => {
        ...
        if (lowerText.includes("scan-target")) {
            const parts = lowerText.split("scan-target");
            const targetPhrase = parts.length > 1 ? parts[1].trim().split(/\s+/) : [];
            const target = targetPhrase[0];

            if (target) {
                const scanResult = simulateVulnerabilityScan(target);
                addMessage(scanResult, false);
            } else {
                addMessage("Bot: Please provide a target for the scan (e.g., scan-target example.com).", false);
            }
            return;
        }
        ...
        if (answer) {
            ...
            const alerts = analyzeText(text);

            if (alerts.length > 0) {
                addAlertBox(alerts);
            } else {
                addMessage("✅ No cybercrime indicators detected. You can continue chatting safely.", false);
            }
        }
    }, 500);
}
addMessage("Hello! I'm your Cybercrime Detection Assistant. Ask me questions or use <strong>scan-target [URL]</strong> to check for simulated vulnerabilities.", false);
